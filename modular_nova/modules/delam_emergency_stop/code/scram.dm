...
/obj/machinery/atmospherics/components/unary/delam_scram/proc/prereq_check(source, trigger_reason)
	...
	if(world.time - SSticker.round_start_time > 600 MINUTES && trigger_reason != DIVINE_INTERVENTION)
		playsound(src, 'sound/machines/compiler/compiler-failure.ogg', 100, FALSE, MACHINE_SOUND_RANGE, ignore_walls = TRUE, use_reverb = TRUE, falloff_distance = MACHINE_SOUND_FALLOFF_DISTANCE)
		audible_message(span_danger("[src] makes a series of sad beeps. The internal charge only lasts about 600 minutes... what a feat of engineering!"))
...
/obj/machinery/button/delam_scram/attack_hand(mob/user, list/modifiers)
	...
	if(world.time - SSticker.round_start_time > 600 MINUTES)
		...
		audible_message(span_danger("[src] makes a series of sad beeps. The internal charge only lasts about 600 minutes... what a feat of engineering! Looks like it's all on you to save the day."))
		burn_out()
		return
